    ...
    (multiple-value-bind (dx dy layout)
        (cond ((carvedp n)
              ...
              ((carvedp w)
               (values -1 0 (layout :v-sense)))
              (t
               (return-from uncarve-dead-end nil)))
      (multiple-value-bind (x y) (stage-coords nh dx dy)
        (loop :for cell = (valid-cell-p stage x y)
              :for new-nh = (and cell (cell-nh stage cell layout))
              :for carved = (and cell (nfilter new-nh #'carvedp))
              :until (or carved (not cell))
